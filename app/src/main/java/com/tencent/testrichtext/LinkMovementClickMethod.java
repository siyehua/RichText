...
public class LinkMovementClickMethod extends LinkMovementMethod {
    private static final String TAG = "LinkMovementClickMethod";
    ...
    public boolean onTouchEvent(TextView widget, Spannable buffer, MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP ||
            ...
            if (link.length != 0) {
                if (action == MotionEvent.ACTION_UP) {
                    //检查是否点击到了空白区域
                    boolean clickTransparentArea = line == widget.getLineCount() - 1 && x > layout.getLineRight(widget.getLineCount() - 1);
                    Log.d(TAG, "touch area: " + clickTransparentArea);
                    if (System.currentTimeMillis() - lastClickTime < CLICK_DELAY && !clickTransparentArea) {
